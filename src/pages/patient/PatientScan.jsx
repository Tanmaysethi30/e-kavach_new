...
import { lookupPatientInRegistry, parsePatientFromQrString } from '../../utils/emergencyRegistry';
...
export default function PatientScan() {
  ...
  const handleManualLookup = () => {
    const key = manualId.trim();
    if (!key) {
      showToast('Please enter a valid Emergency ID, ABHA number, or health token.');
      return;
    }

    // Lookup from emergency registry (which stores active profiles from automatic QR generation)
    const record = lookupPatientInRegistry(key);
    if (record) {
      setScannedPatient(record);
      setScanSimulated(true);
      showToast(`Record retrieved for ${record.name || 'Patient'} via Emergency Registry (${record.emergencyId || key}).`);
    } else {
      // Default fallback simulation if not registered yet
      setScannedPatient({
        name: 'Rajesh V. Sharma',
        abhaNumber: key.includes('-') ? key : '9824-8819-3320-TN',
        emergencyId: key.startsWith('EK-') ? key : 'EK-EMG-9824-8819',
        bloodGroup: 'O+',
      ...
      setScanSimulated(true);
      showToast(`Record ${key} retrieved and verified via ABDM Gateway.`);
    }
  };

  const handleSimulateScan = () => {
    // Resolve Rajesh or current patient from registry
    const record = lookupPatientInRegistry('EK-EMG-9824-8819') || lookupPatientInRegistry('9824-8819-3320-TN');
    if (record) {
      setScannedPatient(record);
    } else {
      setScannedPatient({
    ...
    setScanSimulated(true);
    showToast('Optical scan simulation successful. Emergency Patient QR resolved.');
  };
  ...
  const displayName = scannedPatient?.name || scannedPatient?.fullName || 'Rajesh V. Sharma';
  const displayAbha = scannedPatient?.abhaNumber || scannedPatient?.id || '9824-8819-3320-TN';
  const displayEmergencyId = scannedPatient?.emergencyId || 'EK-EMG-9824-8819';
  const displayBlood = scannedPatient?.bloodGroup ? `${scannedPatient.bloodGroup} Rh Pos` : 'O+ Rh Pos';
  const displayAgeGender = `${scannedPatient?.age || '42 Y'} / ${scannedPatient?.gender || 'Male'}`;
  const displayAllergy = scannedPatient?.allergies || 'Severe Allergy: Penicillin';
  const displayInitials = displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'PT';
        ...
        <div className="flex flex-col gap-space-xl max-w-7xl mx-auto w-full pb-space-2xl">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-space-md pt-space-xs">
            <div className="flex flex-col gap-space-2xs">
              ...
              <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
                Scan a patient's ABHA QR health token or Emergency Health Pass to instantly trigger trauma triage telemetrics and retrieve protected clinical summaries.
              </p>
            </div>
            {/* Action pill */}
            <div className="flex items-center gap-space-sm self-start md:self-auto">
              <button
                className="inline-flex items-center gap-2 px-space-md h-10 rounded-lg bg-surface-container-lowest text-primary shadow-sm hover:bg-surface-container-low transition-colors cursor-pointer"
                id="manual-focus-btn"
                onClick={() => {
                  document.getElementById('abha-input')?.focus();
                  showToast('Focusing manual ID input.');
                }}
                type="button"
              >
            ...
            <div className="bg-surface-container-lowest rounded-2xl p-6 sm:p-10 shadow-xl flex flex-col items-center text-center">
              ...
              <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-space-xs pb-space-md mb-space-md">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-tertiary-fixed text-on-tertiary-fixed font-label-sm text-label-sm font-semibold tracking-wide">
                  ...
                  <span>Ready for Ingress • 60 FPS Optical</span>
                </div>
              </div>
              ...
              <div className="relative w-full max-w-[420px] aspect-[4/3] bg-surface-container-highest/60 rounded-xl flex items-center justify-center overflow-hidden group shadow-inner">
                ...
                <div className="relative z-10 flex flex-col items-center gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="font-label-lg text-label-lg text-primary font-semibold">Position QR within frame</span>
              ...
              <div className="mt-space-lg flex flex-wrap items-center justify-center gap-space-sm w-full">
                <button
                  onClick={() => {
                    ...
                    showToast(cameraActive ? 'Optical camera closed.' : 'Optical 60 FPS Camera feed connected.');
                  }}
                  className="inline-flex items-center justify-center gap-2 px-space-lg h-11 rounded-lg bg-primary text-on-primary font-label-lg text-label-lg font-medium shadow-md hover:bg-primary-container transition-all cursor-pointer"
                  id="open-cam-btn"
                  type="button"
                >
                  <span className="material-symbols-outlined text-[20px]">{cameraActive ? 'videocam_off' : 'videocam'}</span>
                  <span>{cameraActive ? 'Disconnect Optical Camera' : 'Tap to Open Optical Camera'}</span>
                </button>
                <button
                  onClick={() => showToast('Pairing with Bluetooth scanner HW-BT-8891... Paired successfully (Latency 8ms).')}
                  className="inline-flex items-center justify-center gap-2 px-space-md h-11 rounded-lg bg-surface-container text-on-surface-variant font-label-lg text-label-lg hover:bg-surface-container-high transition-colors cursor-pointer"
                  type="button"
                >
                ...
                <button
                  className="inline-flex items-center justify-center gap-1.5 px-space-md h-11 rounded-lg bg-secondary-container text-on-secondary-fixed-variant font-label-lg text-label-lg font-semibold hover:bg-secondary-fixed transition-all cursor-pointer"
                  id="simulate-scan-btn"
                  onClick={handleSimulateScan}
                  type="button"
                >
                  <span className="material-symbols-outlined text-[18px]">bolt</span>
                  <span>Simulate ER Bay Scan</span>
                </button>
              </div>
              ...
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-space-md">
                Instantly pulls verified allergy status, blood type, chronic conditions, and emergency contacts upon scan.
              </p>
            </div>
          </div>
          ...
          <div className="w-full max-w-3xl mx-auto bg-surface-container-low rounded-xl p-space-md flex flex-col md:flex-row items-center justify-between gap-space-md" id="manual-entry-section">
            <div className="flex items-center gap-space-sm w-full md:w-auto">
              ...
              <div className="flex flex-col">
                <span className="font-label-lg text-label-lg font-semibold text-primary">Manual Emergency / ABHA Ingress</span>
                <span className="font-body-sm text-body-sm text-on-surface-variant">Enter Emergency ID (e.g. EK-EMG-9824-8819) or ABHA number</span>
              </div>
            </div>
            <div className="flex items-center gap-2 w-full md:w-auto flex-1 md:max-w-md">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full h-10 px-space-sm bg-surface-container-lowest rounded-lg font-body-md text-body-md text-on-surface placeholder:text-outline outline-none focus:ring-2 focus:ring-primary shadow-sm uppercase tracking-wider font-mono text-[13px]"
                  id="abha-input"
                  placeholder="e.g. EK-EMG-9824-8819 or 9824-8819-3320-TN"
                  type="text"
                />
              </div>
              <button
                className="h-10 px-space-md rounded-lg bg-primary text-on-primary font-label-lg text-label-lg font-medium hover:bg-primary-container transition-colors whitespace-nowrap shadow-sm cursor-pointer"
                id="lookup-btn"
          ...
          {/* Scanned Patient Feedback Modal / Live Result Drawer */}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-space-sm">
              <div className="flex items-center gap-space-sm">
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-headline-sm text-headline-sm text-primary font-bold">{displayName}</span>
                    ...
                    <span className="font-label-sm text-label-sm px-2 py-0.5 rounded bg-secondary-container text-on-secondary-container font-mono font-bold">
                      {displayEmergencyId}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-on-surface-variant font-body-sm text-body-sm flex-wrap">
                    <span>{displayAgeGender}</span>
                    <span>•</span>
                    <span className="font-semibold text-primary">Blood: {displayBlood}</span>
                    <span>•</span>
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-error-container text-on-error-container font-label-sm text-label-sm font-bold shadow-sm">
                  ...
                  <span>{displayAllergy.includes('Allergy') ? displayAllergy : `Allergy: ${displayAllergy}`}</span>
                </div>
            ...
            <div className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tr className="hover:bg-surface-container-low/70 transition-colors bg-surface-bright">
                      ...
                      <td className="py-4 px-space-md">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-[#E4E4FB] text-[#2c2e68] font-label-sm text-label-sm font-semibold">
                          ...
                          <span>Diabetic Protocol • Type II DM</span>
                        </span>
                      </td>
                      <td className="py-4 px-space-md text-right whitespace-nowrap">
                        <Link to="/patient/health-history" className="inline-flex items-center gap-1 px-space-md py-1.5 rounded-lg bg-primary text-on-primary font-label-md text-label-md font-medium hover:bg-primary-container transition-all">
                    ...
                    <tr className="hover:bg-surface-container-low/70 transition-colors">
                      ...
                      <td className="py-4 px-space-md">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-tertiary-fixed text-on-tertiary-fixed font-label-sm text-label-sm font-semibold">
                          ...
                          <span>Cardiac Stent Routine</span>
                        </span>
                      </td>
                      <td className="py-4 px-space-md text-right whitespace-nowrap">
                        <Link to="/patient/health-history" className="inline-flex items-center gap-1 px-space-md py-1.5 rounded-lg bg-primary text-on-primary font-label-md text-label-md font-medium hover:bg-primary-container transition-all">
              ...
              <div className="bg-surface-container-low px-space-md py-3 flex flex-col sm:flex-row items-center justify-between gap-2 text-on-surface-variant font-body-sm text-body-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>ABDM Telemetry Gateway Node #09: Synced with National Health Authority at 100% integrity</span>
                </div>
                <div className="flex items-center gap-4 font-mono text-[11px]">
                  <span>TOTAL TODAY: 38 SCANS</span>
                  <span>•</span>
                  <span>0 MISREADS</span>
                </div>
              </div>
            </div>
          </div>
