...
export default function AbhaHealthId() {
  ...
  const nameVal = currentUser?.name || currentUser?.fullName || 'Registered User';
  ...
  const handleDownloadCard = () => {
    showToast(`Downloading Digital ABHA Card (${abhaVal}.pdf)...`);
    ...
    a.href = url;
    a.download = `ABHA-Card-${nameVal.replace(/\s+/g, '-')}.pdf`;
...
<span className="font-headline-md text-headline-md text-primary font-bold tracking-wider" id="abhaNumberVal">{abhaVal}</span>
<button onClick={() => handleCopy(abhaVal, '14-Digit ABHA ID')} className="p-1.5 rounded-lg bg-surface-container-lowest text-on-surface-variant hover:text-primary hover:bg-surface-container-highest transition-colors cursor-pointer" title="Copy ABHA ID" type="button">
<span className="material-symbols-outlined text-[18px]">content_copy</span>
</button>
</div>
<span className="font-body-sm text-body-sm text-on-surface-variant flex items-center gap-1.5 flex-wrap">
<span className="material-symbols-outlined text-[14px] text-tertiary">badge</span>
  Holder: <span className="font-semibold text-primary">{nameVal}</span>
  <span>•</span>
  Aadhaar: <span className="font-medium text-on-surface">{aadhaarVal}</span>
</span>
</div>
<div className="bg-surface-container-low rounded-xl p-space-md flex flex-col justify-between group hover:bg-surface-container transition-colors">
<div className="flex items-center justify-between">
...
<div className="mt-space-sm mb-space-xs flex items-center justify-between">
<span className="font-headline-md text-headline-md text-primary font-bold tracking-tight" id="abhaAddressVal">{phrVal}</span>
<button onClick={() => handleCopy(phrVal, 'ABHA PHR Address')} className="p-1.5 rounded-lg bg-surface-container-lowest text-on-surface-variant hover:text-primary hover:bg-surface-container-highest transition-colors cursor-pointer" title="Copy PHR Handle" type="button">
<span className="material-symbols-outlined text-[18px]">content_copy</span>
</button>
</div>
